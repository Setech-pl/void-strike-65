...
| Heavy-class C window (`HYBRID_C_HEAVY`, roadmap 4.5a candidate) | 0 of 243 | `$7E12-$7F04`; since the 4.5M-M2 candidate the image rides the tail of the merged low-C/GLUE cold record at `$9D32` (transport capacity 44 B until the M3 arena; the window keeps 243 B) and is published by one disjoint copy from the `stage_glue_holding` tail, reached from `publish_director_abi` before ENTITY expansion (the 4.5M-M1 `$7E38` staging and ascending copy are gone; see memory-map) |
