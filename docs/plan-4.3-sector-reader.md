 ...
 ├────────┼───────┼────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
 │      3 │     1 │ level id (1-16; the campaign fills 1-12, ids 13-16 stay dead)                                                                                                │
 ├────────┼───────┼────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
 │      4 │     1 │ sector count                                                                                                   │
 ├────────┼───────┼────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
 ...
 Resident level directory (16 × 3 B = 48 B, in the reader record) [NOTE 2026-09-23: these figures STAY. Owner decision AC sets the 1.0 campaign at TWELVE levels, but `LEVEL_MAX_ID = 16` (`src/hybrid/sector-reader.s:131`, asserted at `:903`, generated by `scripts/build.mjs:622`) is deliberately left at sixteen — the code indexes sixteen, the data delivers twelve, and the four surplus entries carry a zero `count` that `sector_reader_lookup` already rejects without touching SIO. Trimming to 12 × 3 B = 36 B would free 12 B resident and recompute the runtime evidence and boot baseline; it is a backlog memory reclaim, to be taken when bytes run short]: sector_lo, sector_hi, count per level; entry 0 unused or level 1 at index 0 — executor's choice, asserted by a test. Generated by the build from the fixed level base sector (§7), so its values never feed back into the transport size.
