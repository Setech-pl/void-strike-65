 ...
 Resident level directory (16 × 3 B = 48 B, in the reader record): sector_lo, sector_hi, count per level; entry 0 unused or level 1 at index 0 — executor's choice, asserted by a test. Generated by the build from the fixed level base sector (§7), so its values never feed back into the transport size.
