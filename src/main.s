...
; Menu background stars (owner decision A, 2026-09-22). Seven display-list
; lines that were blank-8 ($70) become ANTIC 4 LMS rows ($44) so they can carry
; star glyphs: two above the title, three in the band between EXIT and the hint
; and two below the hint. A blank-8 line and an ANTIC 4 row are both eight
; scanlines, so the menu's 216-scanline total does not move. Five rows that
; already exist carry the side stars with no display-list change at all - the
; two bar rows, the two PlayerFighter rows and the blank $C4 DLI row.
;
; These rows sit directly after the existing menu bytes, inside the frontend's
; own $4050-$43FF, and clear_screen already covers $4000-$43FF - which is why
; entering OPTIONS and coming back restores the sky for free.
MAIN_MENU_STAR_ROW_0 = 340      ; above the title
MAIN_MENU_STAR_ROW_1 = 380
MAIN_MENU_STAR_ROW_2 = 420      ; between EXIT and the hint line
MAIN_MENU_STAR_ROW_3 = 460
MAIN_MENU_STAR_ROW_4 = 500
MAIN_MENU_STAR_ROW_5 = 540      ; below the hint line
MAIN_MENU_STAR_ROW_6 = 580
MAIN_MENU_STAR_SCREEN_BYTES = MAIN_MENU_STAR_ROW_6 + 40

.include "menu-stars.inc"

; The generator computes every star address from $4000; the runtime only walks
; what it emitted, so a moved SCREEN would silently scatter the sky.
.assert SCREEN = $4000, error, "menu star addresses are generated against SCREEN = $4000"
; The frontend charset is one aligned 1 KiB bank, so ANTIC 4 reaches codes
; 0-127. 1-43 are the font, 48-63 the H3.1 graphics; 64-71 are the star dots.
.assert MENU_STAR_GLYPH_BASE > FRONTEND_GRAPHICS_BASE + 19, error, "star glyphs collide with the H3.1 extended glyphs"
.assert MENU_STAR_GLYPH_BASE + 8 <= 128, error, "star glyphs leave the ANTIC 4 charset"
.assert MENU_STAR_TWINKLE_COUNT < MENU_STAR_COUNT, error, "at least one menu star must stay steady"
...
@restore_palette:
    jsr music_tick
    jsr menu_star_frame_tick    ; restores the palette, then twinkles the sky
:
frontend_input_poll:
    lda STICK0
    and #$0F
...
;
; Frontend only. The MAIN MENU is the single screen that draws them; OPTIONS,
; TOP SCORES, GAME OVER and PAUSE are untouched.
;
; The four parallel arrays are generated by scripts/frontend-h31-assets.mjs from
; the seed in assets/graphics/frontend-h31.json, so the sky is fixed at build
; time and identical on every boot. Twinkling stars occupy the head of the
; arrays, which is the whole reason the per-frame tick is cheap: it walks
; MENU_STAR_TWINKLE_COUNT records and stops, while the one-shot draw walks all
; MENU_STAR_COUNT.
;
; A twinkling star's dim step is its bright glyph | MENU_STAR_DIM_BIT, which
; turns bit pair 01 (COLPF0 $0E, white) into bit pair 11 (COLPF2 $84, steel).
; set_frontend_standard_palette spends all four playfield registers on the
; existing menu and the brief rules out a new DLI, so there is no fifth register
; for a dim white: the twinkling third is drawn from the white stars and steel
; is its dim step. Steel stars stay steady. The 60/40 tone mix is unaffected.
;
; Owner decision A pays for these bytes out of the gameplay starfield expansion
; reservation; see docs/memory-map.md.
.segment "STARFIELD"

menu_star_screen_low:
    EMIT_MENU_STAR_LOW
; Bright glyph in bits 0-2 and 6 (64-67 white, 68-71 steel; twinkling entries
; are always white) and the screen page in bits 3-4, so one array carries both
...
menu_star_phase_end:
; One dot per glyph, four positions. The offset is shape*8 + scanline inside the
; eight-byte glyph; the value is the bit pair placed at the dot's pixel column.
menu_star_shape_offset:
menu_star_shape_tables:
    EMIT_MENU_STAR_SHAPES
menu_star_shape_white = menu_star_shape_tables + 4
menu_star_shape_steel = menu_star_shape_tables + 8
menu_star_shape_tables_end:

.assert menu_star_phase_end - menu_star_phase = MENU_STAR_TWINKLE_COUNT, error, "one phase per twinkling star"
.assert menu_star_shape_tables_end - menu_star_shape_tables = 12, error, "four star shapes, three bytes each"
.segment "PICKUP_CODE"

; The cycle table and the frame counter live in the pickup window rather than in
; the starfield reservation: the packed starfield rides directly below the pickup
...
; VCOUNT $70 (scanline 226), past the 216-scanline menu display, so no cell is
; written while ANTIC is fetching it.
menu_star_frame_tick:
    jsr set_frontend_standard_palette ; restore after the footer DLI
; Falls through. MEASURED cost is stated in docs/STATUS.md; it touches at most
; MENU_STAR_TWINKLE_COUNT cells and nothing else in the frame.
menu_star_tick:
    ldx menu_star_frame
    inx
...
:
    stx menu_star_frame
    ldx #MENU_STAR_TWINKLE_COUNT-1
@star:
    ; The destination first, while the glyph byte's page bits are still needed;
    ; A then becomes the value to store.
    lda menu_star_glyph,x
    lsr
    lsr
    lsr
    and #$03
...
:
    ...
    bmi @off
    ora menu_star_glyph,x       ; $00 keeps bright, $04 selects the steel dot
    and #MENU_STAR_GLYPH_MASK   ; drop the page bits
    bne @store                  ; a star glyph is never zero
@off:
    lda #CH_FRONT_SPACE
@store:
    ldy #$00
...
; Main-menu background stars, the one-shot half. This pair and the two bytes
; draw_main_menu_scene spends reaching it fit inside the alignment padding that
; already precedes the display list below, and the fourteen bytes the list
; itself grew fit inside the padding in front of options_display_list - so
; ENTITY_CODE does not grow, no address past $9500 moves, and MAIN is
; byte-identical. Both run with display DMA off, once per menu entry.
build_menu_star_glyphs:
    ; Eight one-dot glyphs over the cleared frontend charset codes 64-71: four
    ; dot positions in bit pair 01 (COLPF0 $0E white) and the same four in bit
    ; pair 11 (COLPF2 $84 steel), so steel sits exactly MENU_STAR_DIM_BIT above
    ; white and a twinkling star dims with one ORA.
    ldx #$03
@star_glyph:
    ldy menu_star_shape_offset,x
    lda menu_star_shape_white,x
    sta FRONTEND_CHARSET+MENU_STAR_GLYPH_BASE*8,y
    lda menu_star_shape_steel,x
    sta FRONTEND_CHARSET+(MENU_STAR_GLYPH_BASE+4)*8,y
    dex
    bpl @star_glyph
; Falls through. clear_screen has already blanked $4000-$43FF, so every cell
; that is not a star already holds the blank screen code.
draw_menu_stars:
    ldx #MENU_STAR_COUNT-1
@star:
    lda menu_star_glyph,x
    lsr
    lsr
    lsr
    and #$03                    ; the screen page, 0-2
    ora #>SCREEN
    sta dst_ptr+1
    lda menu_star_screen_low,x
    sta dst_ptr
...
main_menu_display_list:
    .byte $44,<(SCREEN+MAIN_MENU_STAR_ROW_0),>(SCREEN+MAIN_MENU_STAR_ROW_0)
    .byte $44,<(SCREEN+MAIN_MENU_STAR_ROW_1),>(SCREEN+MAIN_MENU_STAR_ROW_1)
    .byte $70                    ; clearance row above the title
    .byte $47,<SCREEN,>SCREEN
    ...
    .byte $70                    ; clearance row under EXIT
    .byte $44,<(SCREEN+MAIN_MENU_STAR_ROW_2),>(SCREEN+MAIN_MENU_STAR_ROW_2)
    .byte $44,<(SCREEN+MAIN_MENU_STAR_ROW_3),>(SCREEN+MAIN_MENU_STAR_ROW_3)
    .byte $44,<(SCREEN+MAIN_MENU_STAR_ROW_4),>(SCREEN+MAIN_MENU_STAR_ROW_4)
    .byte $C4,<(SCREEN+260),>(SCREEN+260)
    .byte $42,<(SCREEN+300),>(SCREEN+300)
    .byte $70                    ; clearance row under the hint
    .byte $44,<(SCREEN+MAIN_MENU_STAR_ROW_5),>(SCREEN+MAIN_MENU_STAR_ROW_5)
    .byte $44,<(SCREEN+MAIN_MENU_STAR_ROW_6),>(SCREEN+MAIN_MENU_STAR_ROW_6)
    .byte $70
...
; Owner names (how-to-play.md): ROOKIE / PILOT / ACE, easiest first. Six
; cells each, right-aligned; the internal DIFFICULTY_EASY/MEDIUM/HARD symbols
; and their order are unchanged.
difficulty_value_table:
    .byte CH_FRONT_A+17|ANTIC67_COLOR_PF1,CH_FRONT_A+14|ANTIC67_COLOR_PF1,CH_FRONT_A+14|ANTIC67_COLOR_PF1
    .byte CH_FRONT_A+10|ANTIC67_COLOR_PF1,CH_FRONT_A+8|ANTIC67_COLOR_PF1,CH_FRONT_A+4|ANTIC67_COLOR_PF1
    .byte CH_FRONT_SPACE,CH_FRONT_A+15|ANTIC67_COLOR_PF1,CH_FRONT_A+8|ANTIC67_COLOR_PF1
    .byte CH_FRONT_A+11|ANTIC67_COLOR_PF1,CH_FRONT_A+14|ANTIC67_COLOR_PF1,CH_FRONT_A+19|ANTIC67_COLOR_PF1
    .byte CH_FRONT_SPACE,CH_FRONT_SPACE,CH_FRONT_SPACE
    .byte CH_FRONT_A|ANTIC67_COLOR_PF1,CH_FRONT_A+2|ANTIC67_COLOR_PF1,CH_FRONT_A+4|ANTIC67_COLOR_PF1
