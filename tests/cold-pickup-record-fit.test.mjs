...
test("cold pickup record excludes the source-only character phase bank", () => {
  ...
  assert.equal(packed.length, 854);
  assert.equal(0x917d - 0x8c80, 1277);
  assert.equal(1277 - packed.length, 423);
  ...
  assert.equal(manifest.entityEffects.pickupPhasePackedBytes, 854);
  assert.equal(manifest.entityEffects.pickupPhaseExternalChunk.coldCapacityBytes, 1277);
  assert.equal(manifest.entityEffects.pickupPhaseExternalChunk.coldMarginBytes, 423);
...
test("combined PMG and central primitive retain one legal runtime block", () => {
  const symbol = labels();
  assert.equal(symbol.get("__PICKUP_CODE_RUN__"), 0x8800);
  assert.equal(symbol.get("__PICKUP_CODE_SIZE__"), 871);
  assert.equal(symbol.get("lower_cell_primitive_end") - symbol.get("lower_cell_read"), 187);
  assert.equal(manifest.capitalPlayerCollisionRuntime.packedStreamOffset, 871);
  ...
  assert.equal((source.match(/jsr lower_cell_(?:read|write)/g) ?? []).length, 0,
    "fit proof must not install writer hooks");
});
...
test("fit preserves the reviewed staging and placement gates", () => {
  ...
  assert.equal(manifest.starfieldRuntime.packedSourceToPickupMarginBytes, 10);
  assert.equal(manifest.entityEffects.stagingToBroadsideMarginBytes, 66);
