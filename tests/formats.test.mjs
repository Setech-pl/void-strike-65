...
test("resident compaction proof survives and Spread Shot leaves at least 64 source-owned bytes", () => {
  ...
  assert.deepEqual([
    ...
    layout.suffixRawBytes - layout.suffixPackedBytes]);
  assert.equal(layout.suffixRawBytes - layout.suffixPackedBytes, 1065);
  ...
  assert.equal(reserve.residentSuffixGrossSavingsBytes, 1065);
  ...
  ], [0x5323, 0x535a, 0x5e0d, 0x5e10, 55, 3]);
  ...
  ], [0x552a, 0x5de5, 0x552a, 0x5de5, 2235]);
  assert.match(source,
