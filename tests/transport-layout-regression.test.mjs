...
test("packed startup and relocated GLUE have pairwise-safe real lifetimes", () => {
  ...
  assert.deepEqual({
    ...
    residentPacked: 6690,
    starfieldPacked: 1819,
    a2: 254,
    entityPacked: 2805,
    initialSourcesEnd: 0x53da,
    contentEnd: 0x53de,
  });
  ...
  const ranges = [
    ...
    interval(0x8800, 2031, 11, 14, "pickup/phase/collision runtime"),
  ...
  for (let left = 0; left < ranges.length; left += 1) {
    for (let right = left + 1; right < ranges.length; right += 1) {
      const names = new Set([ranges[left].name, ranges[right].name]);
      const safeBackwardSelfCopy = names.has("entity packed source") &&
        names.has("entity staging");
      assert.equal(overlaps(ranges[left], ranges[right]) &&
        lifetimesOverlap(ranges[left], ranges[right]) && !safeBackwardSelfCopy, false,
  ...
  assert.equal(initialSourcesEnd - 0x5318, 194,
    "the exact source/staging overlap is frozen and copied backwards");
  assert.equal(0x5318 + build.entityPacked.length, 0x5e0d,
    "ENTITY staging must leave its frozen three-byte BROADSIDE margin");
